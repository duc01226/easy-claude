...
> **MANDATORY IMPORTANT MUST ATTENTION — NO EXCEPTIONS:** If NOT already in a workflow, use ask the user directly first. Do NOT judge the task "simple enough to skip"; the user chooses:
>
...
**MANDATORY IMPORTANT MUST ATTENTION — NO EXCEPTIONS** after this skill, use ask the user directly to present these options. Do NOT skip because the task seems "simple" or "obvious"; the user decides:

- **"$test (Recommended)"** — Run tests after changelog update
- **"$docs-update"** — Update docs if needed
- **"Skip, continue manually"** — user decides

> **[IMPORTANT]** Use task tracking to break ALL work into small tasks BEFORE starting, including each file read; this prevents context loss. For simple tasks, AI MUST ATTENTION ask the user whether to skip.
...
**IMPORTANT MUST ATTENTION** before execution, if outside a workflow, use ask the user directly for the user's choice (`workflow-feature` or `$changelog`); after completion, use it again to offer `$test`, `$docs-update`, or manual continuation — why: workflow and follow-up choices belong to the user.
