...
2. **Run the workflow INLINE:** invoke `$workflow-review-changes` via the skill invocation (NEVER the `spawn_agent` tool) with the recomputed `{scope}` as its prompt. Let it run its full 19-step sequence including its own internal fix→re-review loop.
