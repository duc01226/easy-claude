...
- **Bounded:** round cap default 2; blocking findings not shrinking across 2 rounds, or cap hit with CRITICAL/HIGH/MEDIUM fixes still landing → **STOP & escalate** by asking the user directly.
...
- **Round cap (default 2)** and **findings-increasing → STOP & escalate** by asking the user directly. NEVER loop open-ended. Cap exhaustion escalates only when CRITICAL/HIGH/MEDIUM fixes are still landing — a LOW-only round converges via the severity floor.
...
2. **Resolve/create the Goal Contract** per `SYNC:goal-contract-satisfaction-loop` (`plans/goals/{YYMMDD-HHmm}-{slug}/goal.md`, template `.claude/templates/goal-contract-template.md`). Its single **required** Success Criterion:
   ...
   Record the round cap (default 2), the severity floor (LOW non-blocking from round 2), and the scope string in **Constraints**.
...
**IMPORTANT MUST ATTENTION** enforce the **round cap (default 2)**; blocking findings not shrinking across 2 rounds, or cap hit with CRITICAL/HIGH/MEDIUM fixes still landing → **STOP & escalate** by asking the user directly. NEVER loop open-ended.
