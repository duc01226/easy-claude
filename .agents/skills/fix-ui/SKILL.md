...
**Active-goal read (BEFORE fixing):** resolve the active Goal Contract per `SYNC:goal-contract-satisfaction-loop` (active plan `goal.md` → `plans/goals/{YYMMDD-HHmm}-{slug}/goal.md` → create from the reported UI issue). UI fix closure requires Goal Satisfaction evidence in the Iteration Log: screenshot reference (parent container, not full page), viewport(s) verified, interaction proof, and accessibility proof when applicable. Closure is blocked while any required goal criterion remains FAIL. Never store private screenshots or customer data in goal files — reference report paths.
