...
**IMPORTANT MUST ATTENTION Workflow:** Classify the source scope → `$investigate` and record `file:line`$research evidence → `$excalidraw-diagram` to create the visual diagram → verify source fidelity and output quality → `$workflow-end`; invoke every non-skipped step through its skill invocation and preserve task evidence.
