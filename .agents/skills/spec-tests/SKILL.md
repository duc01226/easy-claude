---
name: spec-tests
description: '[Testing] Use when you need to generate or update test specifications in feature docs (Section 8) with unified TC-{FEATURE}-{NNN} format. Renamed from the former /tdd-spec skill — that name no longer resolves as a slash command.'
---
...
> **Renamed:** formerly `/tdd-spec` — that name no longer resolves as a slash command; use `$spec-tests`.
...
> <!-- /SYNC:cross-cutting-quality -->
>
> > **`.claude/skills/spec-tests/references/spec-tests-template.md`** — TC format template: GWT structure, Evidence field, decade-numbering, Preservation Tests section (mandatory for bugfixes). Read before generating any TC.
>
> - `.claude/skills/spec-tests/references/spec-tests-template.md` — TC template format
...
5. **Test-Code Sync** — Optionally reconcile Section 8 TCs ↔ integration test code (forward-sync; §8 canonical)
...
| Skill                       | Relationship                                                                        |
| --------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `spec-tests [direction=sync]` | **Native sync mode** — forward-syncs Section 8 TCs ↔ integration test code          |
| `integration-test`          | Code generator → generates integration tests FROM TCs written by this skill         |
| `feature-spec`              | Feature doc creator → creates the Section 8 that this skill populates              |
...
| Integration test code        | `{IntegrationTests}/` — §8 TCs forward-synced here via `[direction=sync]` |
...
**If target feature doc missing:** suggest `$feature-spec` first, OR create minimal Section 8 stub.
...
> | Spec was wrong (code implements correct behavior that spec misdescribed) | Spec described behavior that never worked correctly; the “fix” is actually a clarification | **STOP** — do NOT update TCs yet. First: run `$feature-spec [update]` on the affected sections (Section 3, 4, 5 — business rules, user journeys, API contracts) to correct the canonical Feature Spec. THEN return here to update §8 TCs. |
...
6. Update feature docs Section 8 (canonical), then forward-sync to integration test code via `[direction=sync]`
...
4. "$spec-tests [direction=sync] — Sync §8 TCs ↔ integration test code"
5. "Done for now"
```
...
```
1. "$spec-tests [direction=sync] — Sync §8 TCs ↔ integration test code after reconciliation (Recommended)"
...
```
1. "$spec-tests [direction=sync] — Sync §8 TCs ↔ integration test code for newly documented TCs (Recommended)"
...
5. Forward sync (`$spec-tests [direction=sync]`) auto-handles deprecated TCs in Section 8
...
- `review-artifact --type=spec-tests` — TC quality review (use AFTER this skill to validate TC coverage and correctness)
- `spec-tests [direction=sync]` — Native sync mode (forward-syncs Section 8 TCs ↔ integration test code)
...
**Triggered when:** "sync test specs", "sync tests", "reconcile tests", "reverse sync", "full sync", or `[direction=sync|forward|reverse|full]`.
...
>
> 1. **Spec-only review chain** (Recommended) — $spec-tests → $review-artifact --type=spec-tests → $quality-gate
> 2. **`$spec-tests` directly** — standalone

## Next Steps

**[BLOCKING]** After completing, a direct user question — do NOT skip:

- **"$review-artifact --type=spec-tests (Recommended)"** — Validate TC quality (completeness, GWT format, coverage gaps)
...
```
spec-tests (you are here)
  │
  ├─ PREREQUISITE:
  │    [REQUIRED] feature-spec doc must exist at docs/specs/{Bucket}/README.{FeatureName}.md
  │    If not found → run $feature-spec init first
  │
  ├─ CREATE mode (new feature):
  │    spec-tests CREATE → $review-artifact --type=spec-tests → $spec-tests [direction=sync] → $integration-test [from-prompt]
  │
  ├─ UPDATE mode (code changed):
  │    *** Spec-Wrong? Gate first (see above) ***
  │    spec-tests UPDATE → Step UPDATE-FINAL (Blast Radius) → $review-artifact --type=spec-tests → $spec-tests [direction=sync] → $integration-test [from-changes]
  │
  ...
  │     Always run after UPDATE to reconcile §8 TCs with integration test code.
  │
  ...
  └─ [RECOMMENDED] → $docs-update
        For full chain including feature-spec (Phase 2) and spec-index (Phase 2.5).
        Call when multiple doc layers may be stale.
```

### Integration with Bugfix Flow

When spec-tests is called in **REGRESSION mode** (bugfix workflow):

1. Run **Spec-Wrong? Gate** FIRST (same logic as UPDATE mode)
2. If spec was wrong → run `$feature-spec update` (fix the canonical spec) BEFORE writing regression TCs
...
**IMPORTANT MUST ATTENTION** NEVER write TCs to `docs/specs/` as primary destination — Section 8 is canonical.
**IMPORTANT MUST ATTENTION** NEVER generate TCs without reading existing Section 8 — ID collisions corrupt registry.
