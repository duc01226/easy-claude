...
- `status` is `CHANGED` and both `review` and `skip` are `null` → STOP and ask the user with ask the user directly (never commit yet):
  ...
  > Options (in order — first is the default):
  > 1. `Run $changes-review --fix-loop` (Recommended) — do NOT commit yet; activate the `changes-review` skill in `--fix-loop` mode, passing it this exact descriptor JSON (including `"amend":true` for an amend) so its snapshot measures the same candidate. It reviews, validates findings, fixes at the owning layer, and runs a fresh full re-review until it converges, then mints the receipt. Return to this step when it converges (and re-derive if it changed files).
  > 2. `Run $why-review --fix-loop` — same (pass the same exact descriptor JSON), using the rationale-review loop (`why-review` in `--fix-loop` mode); it mints the receipt on convergence.
