...
This skill handles **commit** by default. A plain push delegates to the `git-manager` sub-agent (`agent_type: "git-manager"`), which enforces conventional-commit validation and prevents `--no-verify` bypass. Spawn `git-manager` after committing when the user says "push".
...
> **MANDATORY:** Push operations spawn `git-manager` sub-agent (`agent_type: "git-manager"`), NOT the main agent. Pull-request operations route to the `pull-request` skill (see above).
