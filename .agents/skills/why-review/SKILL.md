...
> **Full-mode sub-agent callers (parallel, report-only):** these rules bind ANY caller that runs this skill in full mode as a sub-agent or as a nested report-only step — you are in this context when you were spawned as a sub-agent, or your brief names you a parallel/barrier member or a report-only occurrence. Examples: a review skill's parallel rationale dimension beside its dimensional reviewers; a workflow's whole-target review occurrence dispatched beside another reviewer; `$plan-review`'s Parallel Review Wave spawns it on EVERY review round beside the core pass and triggered lenses, targeting the plan dir (`plan.md`, `goal.md`, `phase-*.md`). In these runs: never edit the target, never bind the `/goal` gate — record `/goal accelerator unavailable — sub-agent context`, return material Trade-Off / owner-judgment questions UNANSWERED for the caller to ask (never call ask the user directly), skip Next Steps and the `$llm-council` follow-up, and never invoke the caller. Findings validation of the caller's merged report is a separate, sequential `$why-review --validate-findings` invocation the caller owns. **These callers NEVER pass `--fix-loop`** — they are read-only and a sub-agent cannot own an outer fix loop; the `$plan-review` wave member is ALWAYS plain full mode. If `--fix-loop` ever reaches a sub-agent run, refuse it, record `--fix-loop refused — sub-agent context; ran report-only full mode`, and return retained findings for the caller to fix.
