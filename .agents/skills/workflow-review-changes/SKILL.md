...
2. **Resolve/create the Goal Contract** per `SYNC:goal-contract-satisfaction-loop` (carried below; `plans/goals/{YYMMDD-HHmm}-{slug}/goal.md`, template `.claude/templates/goal-contract-template.md`). Its single **required** Success Criterion:
...
2. **Run the workflow INLINE:** invoke `$workflow-review-changes` (default mode, WITHOUT `--fix-loop`) via the skill invocation (NEVER the `spawn_agent` tool) with the recomputed `{scope}` as its prompt. Let it run its full 21-step sequence including its own internal fix→re-review loop.
