...
> **Translation Sync:** The `$review-changes` skill (task #2) includes a **mandatory** multilingual UI translation-sync check. When UI text changes in multilingual projects without locale updates, the skill uses a direct user question for an explicit user decision — NOT purely advisory.
...
After steps 1 and 2 (`$why-review` HOLISTIC and `$review-changes`) complete, spawn all active parallel reviewers in **a single response** with multiple `spawn_agent` tool calls:
