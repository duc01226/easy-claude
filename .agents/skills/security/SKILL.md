...
3. **After validated fixes:** restart the full security review from Scope over the full current security target. If the restarted review needs a fresh reviewer, spawn a NEW `security-auditor` sub-agent (`agent_type: "security-auditor"`) — ZERO memory of prior rounds. Include in prompt: OWASP Top 10 checklist, auth flows, injection risks, dependency CVEs, microservices boundary security.
