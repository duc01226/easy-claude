...
6. **Next Steps** — ask the user directly: `$plan` (fix validated findings) / `$code-simplifier` / skip.
...
**IMPORTANT MUST ATTENTION — Main steps (execute in order, NEVER skip/merge):** (1) Resolve scope (args else ask the user directly; map to each child's args) → (2) Load project reference docs once → (3) Parallel fan-out of all three read-only sub-agents in ONE message behind an all-return barrier → (4) Consolidate + dedup into one report with three sub-scores + one combined verdict → (5) `$why-review` self-validation gate on the merged findings → Next Steps ask the user directly.
