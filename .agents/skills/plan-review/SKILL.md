...
- **Round cap 3 — a ceiling, NEVER a target;** a clean pass ends the loop once the persisted `minRounds` is met. Escalate by asking the user directly when the same blocker survives 2 consecutive full re-reviews with no progress, when round 3 completes with blocking findings still open, or when a finding needs product/owner judgment — cap exhaustion escalates, it NEVER becomes a PASS.
