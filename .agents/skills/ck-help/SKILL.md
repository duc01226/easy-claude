...
Full documentation (config, schema, setup guides). `config` (also `settings`, `options`, `switches`, `env`) prints every `.ck.json` / `.ck.local.json` option with its help text, the adopter quick-settings table and the environment variables — generated by `node .claude/scripts/ck-config-help.cjs` from the settings schema and their owning docs — plus the command that lists every project-config option. If the generated block reports it is unavailable, run that command directly.
