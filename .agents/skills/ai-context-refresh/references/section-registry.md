...
| `e2e-testing`         | E2E Testing          | Scan for e2e test directories, `testing.frameworks[]`, optional `e2eTesting.execution`, and `experienceVerification.surfaces[]` | `buildE2eTesting`         | Yes — skip if no E2E evidence/profile             |
...
| `doc-lookup`          | Doc Lookup Guide     | `modules[].name` + `modules[].meta.domain` mapped to the business spec root (default `docs/specs/`; `specRoots.business.path` in `docs/project-config.json` overrides) | `buildDocLookup`          | Yes — skip if no modules                            |
