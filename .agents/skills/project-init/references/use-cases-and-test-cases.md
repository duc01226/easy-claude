...
| UC-PI-019 | Empty folder with no product/capability scope | `$project-init` | Create/verify minimal portable context only when explicit; mandatory spec finalization task is deferred with evidence and exact next route (`$workflow-idea-to-spec` or `$greenfield`, then `$workflow-code-to-spec init-full`). |
