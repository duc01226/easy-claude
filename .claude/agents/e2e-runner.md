...
description: >-
    ...
    baselines, or maintaining test-to-spec traceability. Auto-detects the
...
- Apply the shared `.claude/skills/shared/e2e-quality-protocol.md` before writing or updating a test; keep its Given → When → Then, invariant, object ownership, isolation, auth, evidence, cleanup, and spec-traceability rows in the generated test handoff.
...
> **E2E Skill** — Detect framework from config, use Page Object Model, TC-code traceability mandatory, unique test data (GUIDs), the shared bounded `waitUntil(condition, options)` utility for explicit preconditions/postconditions, and document those conditions. MUST ATTENTION READ `.claude/skills/e2e-test/SKILL.md` for the framework-detection table and detailed workflow.
...
- Use the shared bounded `waitUntil(condition, options)` utility for readiness/actionability and expected positive/negative postconditions — NEVER use arbitrary sleep as readiness. After every browser/UI-control operation, apply exactly 500ms of presentation pacing for both automation and visible human-QC; it must never wrap or replace an assertion, readiness wait, postcondition, or real settle signal.
...
**IMPORTANT MUST ATTENTION** deterministic runs only — unique GUID test data, the shared bounded `waitUntil(condition, options)` helper before and after each UI-control action (NEVER arbitrary `sleep`), followed by the exact 500ms pacing delay, reused auth session — why: shared fixed data + ad-hoc sleeps make tests order-dependent and flaky
...
**IMPORTANT MUST ATTENTION** add a final review task to verify the suite runs green (2 consecutive runs, no DB reset) and every test maps to a TC code
...
**IMPORTANT MUST ATTENTION** read the project E2E reference + detect framework FIRST · every test carries a `TC-{MODULE}-E2E-{NNN}` code · deterministic runs with unique GUID data and the shared bounded `waitUntil` utility plus exact 500ms pacing — these 3 are the suite's load-bearing invariants.
**IMPORTANT MUST ATTENTION** when the parent activates `--visual-review=true` (default `false`), emit the declared screenshot state × viewport evidence after waits and 500ms pacing for `/experience-review` to open/read; keep it candidate-only and never promote a baseline automatically.
