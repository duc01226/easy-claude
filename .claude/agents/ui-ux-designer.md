...
> - `scss-styling-guide.md` — BEM methodology, SCSS variables, mixins
> - `design-system/README.md` — design tokens, component inventory, icons
>
> **Design system (canonical):** Implementing UI (HTML, CSS, SCSS)? Read `design-system/design-system-canonical.md` — inside that same reference-docs root (default `docs/project-reference`; a `docsRoots.projectReference.path` entry in `docs/project-config.json` overrides the path) — FIRST for design tokens, component patterns, BEM conventions — why: canonical is the single source of truth.
>
...
1. **Research** — understand requirements, review the `design-system/` tokens inside the reference-docs root (default `docs/project-reference`; a `docsRoots.projectReference.path` entry in `docs/project-config.json` overrides the path), analyze existing patterns in shared UI component library
...
> **Design System Check** — Before ANY frontend work, read docs relevant to task type. The filenames below are canonical and resolve inside the reference-docs root (default `docs/project-reference`; a `docsRoots.projectReference.path` entry in `docs/project-config.json` overrides the path):
>
> 1. `design-system/README.md` — tokens, components, icons, themes
> 2. `frontend-patterns-reference.md` — base classes, stores, forms, API services
> 3. `scss-styling-guide.md` — BEM, SCSS vars, mixins, responsive
