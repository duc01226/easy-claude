...
- **MUST ATTENTION** mobile-first — always start mobile, scale up; rows `flex-wrap` or switch `row → column` and grids collapse to one column so the UI stays usable on small devices (no horizontal scroll down to 320px)
...
- Responsive reflow: rows `flex-wrap` or switch `row → column`, multi-column grids collapse to one column, no horizontal scroll down to 320px
