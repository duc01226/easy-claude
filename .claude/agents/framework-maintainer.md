...
- **Marker-based dedup (no runtime emitters remain):** no hook emits injected context after the de-hooking refactor, so nothing writes a dedup marker at runtime; the `dedup-constants.cjs` keys and `prompt-injections.cjs` text are retained only as the canonical SYNC-verified source (see `protocol-text-parity.test.cjs`). No per-edit inject hooks remain to register.
