...
- **External memory / recovery:** state persists in the OS-temp `CK_TMP_DIR` namespaces (`todo/todo-state-{sessionId}.json`, `workflow/{sessionId}.json`), plus swap files and durable `plans/` artifacts, so it survives compaction without becoming repository source; recovery is now skill-driven — static `CLAUDE.md` re-read re-anchors protocol and `/recover` resumes from the actual per-session state (the former `pre-compact-snapshot` → `post-compact-recovery` hooks were removed in the de-hooking refactor). `autoMemoryEnabled: false` — never re-enable Claude's built-in memory; the framework owns state.
...
### Codex mirror sync (18-stage pipeline, 9 verifier scripts)
...
- `npm run codex:sync` = `node .claude/skills/sync-codex/scripts/run-codex-sync.mjs` — **18 sequential stages** (1–3 mutate, 4–18 read-only; configured stages fail-fast, optional capability stages explicitly skip when their contract is absent): `migrate → hooks → context → tests → scripts-tests → tech-spec-freshness → feature-registry → hooks-count-drift → hooks-parity → hooks-doc-sync → wf-cycle → sk-proto → residue → sdd → review-validate-coverage → sync-adoption-parity → provenance-markers → sync-divergence`. The feature-registry stage reads the project's configured canonical roots and includes their continuation parts.
...
6. **Validate** — run available tests (`node .claude/hooks/tests/test-all-hooks.cjs`, `npm run codex:test:tooling`), `python .claude/scripts/generate_catalogs.py --skills` if catalogs changed, and read-only codex verifiers (`--only=...`). Report pass/fail with output.
