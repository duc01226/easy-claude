...
**IMPORTANT MUST ATTENTION Goal:** Deliver subcutaneous integration tests that run through real DI containers against live infrastructure (no HTTP, no mocks) and each map traceably to a `TC-{FEATURE}-{NNN}` spec — every covered behavior verified end-to-end and provably tied to its spec.
