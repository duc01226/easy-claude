"""Inject SYNC:environment-fault-hypothesis marker + reminder into the failure-judgement hosts.

Idempotent — refresh files that already carry the tag, insert into those that don't.
Inserts:
  TOP block:  immediately BEFORE the SYNC region start (per
              sync_blocks.find_sync_region_start), co-locating the marker with the
              reminders region below the main authored content. When a host has no
              SYNC region, the block appends at EOF (creating the host's first one).
  BOTTOM:     a SYNC:...:reminder block immediately BEFORE `## Closing Reminders`,
              or appended at EOF when no such heading exists.

Host set = every carrier that ADJUDICATES a failure — i.e. decides whether a bug,
failing test, error, or unexpected output means the code is wrong. That is exactly
...
`inject(text)` is path-agnostic, so agent files flow through the same logic as
skills. Structure mirrors inject_scenario_stress_gate.py.
"""
...
AGENT_NAMES = [
    "debugger",                       # leaf that renders root-cause verdicts
    "tester",                         # leaf that renders test verdicts
    "integration-tester",             # leaf running suites against shared infrastructure
    "e2e-runner",                     # leaf running browsers — resource pressure is routine here
]

TAG = "SYNC:environment-fault-hypothesis"
REMINDER_TAG = "SYNC:environment-fault-hypothesis:reminder"
TOP_OPEN = f"<!-- {TAG} -->"
