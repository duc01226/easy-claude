...
1. **Edit the matrix** — add/adjust the agent's row in `AGENT_QUALITY_BLOCKS` (or its `FAMILIES` membership) in `agent_protocol_matrix.py`. The row is a **persistent target set**: list every block the agent should carry, including ones it already has — the manifest is the single source of truth for intended per-agent state, so blocks are never pruned after injection. Then `python .claude/scripts/agent_protocol_matrix.py --validate` → must print `OK` (canonical tags, no orchestration leak, partition clean, spawn-capability clear, tier policy clear). The `OK` line reports `N declared block(s), M pending insert(s)` — `M` is the real dry-run count (declared minus already-present), so `M` is **0** at steady state. Check (c) warns only on the inverse condition: a declared block **absent** from its agent, meaning the injector hasn't been run yet.
2. **Dry-run the injector** — `python .claude/scripts/inject_agent_protocol_blocks.py --dry-run` → review the planned adds (count must match the `pending insert(s)` figure from step 1). A bare invocation targets **every** matrix agent; narrow it with `--family=<name>` or `--agents=<a,b>`. There is no `--all` flag — the unknown-argument guard rejects it.
3. **Real run + idempotent re-run** — `python .claude/scripts/inject_agent_protocol_blocks.py`, then re-run `--dry-run` → must report **zero adds** (idempotent). Re-run the `agent-universal-rules` suite (`node .claude/hooks/tests/run-all-tests.cjs`) → `TC-UAR-003..007` green.
