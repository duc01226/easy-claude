...
const SCRIPT_LINES = [
    'Walk through /feature to see how everything connects: scout → investigate → plan → approval gate → implement → spec-tests → integration-test → docs-update. Every approval is explicit — the AI shows the plan and waits for confirmation before writing a single line of code.',
    "The spec-driven loop is mandatory: /idea-to-pbi writes TC-{FEATURE}-{NNN} test cases in Feature Spec Section 8 before development starts. The /feature and /fix workflows both require /spec-tests — code satisfies pre-existing specs, not the other way around. This prevents 'write first, spec later' drift.",
...
const WORKFLOWS = [
    {
        ...
        steps: 'scout → investigate → plan → ✓ approve → implement → spec-tests → docs-update',
    ...
    },
    {
        ...
        steps: 'capture idea → validate scope → refine PBI → DOR gate → spec-tests',
...
const CALLOUTS = [
    {
        ...
        points: [
            ...
            { label: 'Mandatory spec step', text: '/feature and /fix both require /spec-tests — code satisfies specs, not the other way around' }
        ]
    },
    {
