...
> 124 runnable skills across 15+ domains + 10 shared reference/protocol files for context-aware AI assistance (`_templates/template-skill` is a source template, not a runnable skill)
...
> Curated highlights — the full catalog has 124 runnable skills; the tables below list selected skills per domain, not the complete set.
...
_Source: `.claude/skills/` | 124 runnable skills across 15+ domains + 10 shared reference/protocol files (the `_templates/template-skill` source is excluded from runtime discovery)_
