...
| Invoked by `/plan-review`'s Parallel Review Wave (full-mode rationale sub-agent, every round; read-only — NEVER `--fix-loop`) | `plan-review/SKILL.md` → "Parallel Review Wave" (the why-review member's brief carries this deferral as a binding constraint); its Impact-Aware matrix dispatches `/integration-test-review` as a separate wave member when triggered | `Linkage deferred — plan-review Impact-Aware wave owns /integration-test-review.` |
