...
> 3. **The user picks from drafts, never from text.** After presenting the N drafts, open them and ask with `AskUserQuestion`, recommended draft first; never pick for the user while they can be asked (no question tool → ONE draft, auto-selected; drafts unshowable or the question tool errors → AUTO-SELECT the recommended draft, step 9); "continue" is not a pick.
