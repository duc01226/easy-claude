...
**Final Purpose:** Ensure refined PBIs are complete, testable, evidence-backed, and ready for story creation without hidden gaps.
...
Do not spawn a fresh sub-agent just to re-review the same finding set before fixing it. If the PBI needs work, fix actionable findings first, then restart the full PBI review over the current artifact. When that restarted review uses a fresh `general-purpose` sub-agent, use the canonical Agent template from `SYNC:review-protocol-injection` above. PBI artifact reviews are NOT code reviews — use `subagent_type: "general-purpose"`. When constructing the Agent call prompt:
...
4. Set the Task as `"Run a full fresh PBI review after fixes were applied. Focus on: implicit assumptions not validated, missing acceptance criteria coverage, edge cases not addressed, cross-references not verified, vague language, authorization gaps, and regressions introduced by fixes."`
5. Set Target Files as the explicit PBI artifact file path
6. Set report path as `plans/reports/refine-review-rerun{N}-{date}.md`
...
3. **If FAIL:** fix actionable PBI findings, then restart the full PBI review from the beginning
...
**IMPORTANT MUST ATTENTION Final Purpose:** Ensure refined PBIs are complete, testable, evidence-backed, and ready for story creation without hidden gaps.
