...
/**
 * docx-convert dispatcher
 *
 * Routes to one of the two self-contained direction converters:
 *   --to markdown  ->  ../to-markdown/scripts/convert.cjs   (DOCX -> Markdown)
 *   --to docx      ->  ../to-docx/scripts/convert.cjs       (Markdown -> DOCX)
 *
 ...
 *   node scripts/convert.cjs --to docx --help
 *
 * Each direction has its own package.json and node_modules; install the one you need:
 *   cd .claude/skills/docx-convert/to-docx && npm install
 *
 * Note: this dispatcher is intentionally duplicated in pdf-convert rather than shared.
...
DIRECTIONS:
  --to markdown    Convert a Word DOCX file to Markdown
  --to docx        Convert Markdown to a Word DOCX file
...
EXAMPLES:
  node .claude/skills/${SKILL}/scripts/convert.cjs --to markdown --input ./contract.docx
  node .claude/skills/${SKILL}/scripts/convert.cjs --to docx --input ./README.md`);
}
