...
- **No arbitrary round cap;** a clean pass ends the loop immediately. Escalate via `AskUserQuestion` only when the same blocker survives 3 consecutive full re-reviews with no progress, or a finding needs product/owner judgment.
