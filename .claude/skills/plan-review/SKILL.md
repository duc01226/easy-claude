...
| Frontend page, component, store, UX flow, HTML, CSS/SCSS, accessibility, responsiveness, or browser-visible state | **UI Plan Checklist lens** — `ui-ux-designer` sub-agent working `.claude/docs/design-review-checklist.md` against the PLAN (see **UI Plan Checklist Gate**), plus `/ui-review` conventions (design system, styling) when the plan cites concrete files | Per UI phase: Design Plan (`DD-1`–`DD-3`); per new/reshaped view its primary task, container (`E9`), information priority now/later/not-here, inputs at creation vs deferred (`R1`–`R2`), complexity budget (`B15`); design-system conventions; loading, empty, error, and permission states; keyboard/screen-reader behavior incl. dialog focus (`I15`); responsive/long-content layouts; selectors/page objects; and E2E coverage. |
...
## UI Plan Checklist Gate (MANDATORY when any phase creates or reshapes a user-facing surface)

A plan is the cheapest place to stop an overloaded screen: once built, every field has an owner defending it. When the impact matrix triggers the frontend row, the wave MUST include the **UI Plan Checklist lens** — `/ui-review` alone reviews source diffs and finds nothing in a plan that has no code yet.

**Lens brief** (fresh `ui-ux-designer` sub-agent, read-only, same wave as the core pass): READ `.claude/docs/design-review-checklist.md` (§0.1, §0.3 map, §B, §E, §R, and the platform/usage-conditional sections that apply) and `.claude/docs/design-review-calibration.md`, the project design-system/frontend references, the governing spec's interaction layer when one exists (`SYNC:ui-intent-layer`), and every UI-bearing `phase-*.md`. For EACH new or reshaped view in the plan, verify the plan states — and flag the checklist ID it leaves unbound when it does not:

1. **Primary task + container** — the one job of the view and a container that fits it (`B14`, `E9`–`E11`): a long or multi-section entry task in a dialog is a finding.
2. **Information priority** — what the view shows/asks `now`, defers `later`, or leaves `not here` (`B12`, `B13`); must match the governing spec's record when one exists.
3. **Inputs at creation vs deferred** — a draft Field Necessity Matrix for every input-bearing view (`R1`–`R2`, `UI-7.1`); counts vs `uiReview.complexityBudget` when the project declares one (`B15`).
4. **States and accessibility** — the relevant empty/loading/error/permission states (`D2`, `UI-1.5`), the selected accessibility standard, dialog/focus handling for overlays (`I15`), and applicable conditional sections (§F/§G by platform, §H by usage, §L).
5. **Design Plan** — `DD-1`–`DD-3` present or `ADOPTED — {design-system doc}`.
6. **Honesty** — no phase ships a visible control whose backing capability is out of scope without hiding or flagging it (`K10`).

Output `tmp/reports/plan-review-ui-checklist-round{N}-{date}.md`, written incrementally per phase; every finding carries phase file:line, checklist ID, and severity translated through the checklist §0.3 map. Unbound items on a UI phase are real findings, merged and validated like any other lens result. A plan with no user-facing surface records the row `N/A` with evidence.
...
- **Binding constraints (include verbatim):** READ-ONLY — NEVER edit `plan.md`, `goal.md`, `phase-*.md`, or any other file except its own report · NEVER pass or honor `--fix-loop` (it mutates the target; plan-review owns plan fixes) · NEVER bind the `/goal` gate — record `/goal accelerator unavailable — sub-agent context` and rely on why-review's protocol loop · NEVER call `AskUserQuestion` or run why-review's Next Steps — return the questions unanswered · NEVER invoke `/plan-review` (the caller — a callback closes a cycle) · record `Linkage deferred — plan-review Impact-Aware wave owns /integration-test-review.` for the Integration-Test-Review Linkage.
...
- [ ] **UI Plan Checklist Gate (UI phases only):** the UI Plan Checklist lens ran and every new/reshaped view states its primary task, container, information priority, creation-vs-deferred inputs, relevant states, accessibility standard, and Design Plan — each unbound item is a finding with its checklist ID; plans with no user-facing surface record `N/A` with evidence.
...
2. **Integrate** findings as `## Re-Review {N} Findings`, `## Re-Review {N} Why-Review Rationale Findings`, and `## Re-Review {N} Specialist Lens Findings` in the main report — DO NOT filter or override; ask any returned UNANSWERED Trade-Off / owner-judgment questions via `AskUserQuestion` before the verdict
...
└──────────────┬───────────────────────────┘
        ...
        └──────┬──────────────────────────────────┘
               │
               └──→ Loop ENDS at round 2 — bar clear (zero findings round 1; zero CRITICAL/HIGH/MEDIUM round 2), repeated-blocker rule, or the HARD round cap of 2 (no extension; still blocking at round 2 → AskUserQuestion). There is NO round 3 for review blockers — only a failing TEST gate continues past round 2, and that is outside the round budget.
```

### Iteration Rules

1. **Repeated blocker cap** — continue until a complete full review pass clears the round's bar (zero findings in round 1; zero CRITICAL/HIGH/MEDIUM from round 2, LOW-only ends it); if the same blocker repeats across 2 full invocations with no progress, STOP and escalate to user via `AskUserQuestion`
...
- **If the 2-round cap or the repeated blocker cap is reached**: List every remaining issue with `file:line` and severity. STOP. Ask the user to fix, accept, or regenerate the plan via `AskUserQuestion`. NEVER open a third round and NEVER pass instead.
...
<!-- Diverges from canonical `SYNC:double-round-trip-review` on ONE point only: the round budget.
     ...
     the owner via `AskUserQuestion` instead (see `OVERRIDE:review-policy` and
...
<!-- Carrier-local SCOPE NOTE for the canonical `SYNC:severity-rubric` block below — deliberately a
     ...
     escalates to the user via AskUserQuestion". Scoped rather than OVERRIDEn because the divergence
...
> one thing: whether the round exits clean or **escalates to the user via `AskUserQuestion`**. LOW is
...
**IMPORTANT MUST ATTENTION UI plan checklist:** when any phase creates or reshapes a user-facing surface, the wave includes the UI Plan Checklist lens (`ui-ux-designer`, `.claude/docs/design-review-checklist.md` in plan mode): per view primary task, container, information priority, creation-vs-deferred inputs, states, accessibility, Design Plan — why: `/ui-review` reviews code, so without this lens an overloaded screen passes plan review unexamined.
