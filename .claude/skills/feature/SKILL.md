...
**Goal Contract mapping:** resolve the active Goal Contract per `SYNC:goal-contract-satisfaction-loop` (active plan `goal.md` → `plans/goals/{YYMMDD-HHmm}-{slug}/goal.md` → create from the feature request via `.claude/templates/goal-contract-template.md`) at PHASE 1A initialization, and map every feature success criterion to a saved goal criterion. At SUCCESS VALIDATION, append an Iteration Log entry with evidence and mark Goal Satisfaction status — completion requires every required criterion PASS or an escalated blocker.
