...
**IMPORTANT MUST ATTENTION Workflow:** resolve and record the canonical `codebase` or `knowledge` occurrence list → invoke every returned step through its Skill tool → record source evidence, resolver fingerprint, render/validation results, and any conditional skip → `/workflow-end` → `/watzup`.
