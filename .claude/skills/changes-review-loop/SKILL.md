...
**IMPORTANT MUST ATTENTION** enforce the **round cap (default 3)**; blocking findings not shrinking across 2 rounds, increasing, or cap hit with blocking findings still open → **STOP & escalate** via `AskUserQuestion`. NEVER loop open-ended; round-2+ LOW-only findings converge and are recorded as deferred.
