...
5. **DELEGATION (inside a parent workflow):** when this whole workflow runs as a sub-agent, step 13 still runs — it is part of the 16-step sequence — but the parent owns the session `/goal` gate; the sub-agent returns its summary per the return contract.
...
> - **Step 0 `/goal` gate (pre-sequence)** — install at top-level invocation; skip when this whole workflow runs as a sub-agent inside a parent workflow (parent owns the goal). Not one of the 16 counted steps.
...
**IMPORTANT MUST ATTENTION** when invoked as a step inside a parent workflow, run this whole 16-step workflow via `Agent` (`subagent_type: "code-reviewer"`), NEVER inline — why: inline execution absorbs 16 steps of context into the parent session.
...
**IMPORTANT MUST ATTENTION** Step 0 (FIRST ACTION, top-level only) installs the `/goal` self-recursive review-loop gate so stopping is BLOCKED until the whole loop reaches a clean zero-finding pass — a pre-sequence wrapper, not one of the 16 steps; skip it only when this workflow runs as a sub-agent inside a parent workflow (parent owns the goal).
