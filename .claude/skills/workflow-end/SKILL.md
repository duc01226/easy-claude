...
- **Model-driven close:** completes once ALL TaskList items done, the sync gate recorded synced-or-accepted-as-is, and the exact owned baseline run closed successfully or was explicitly not applicable. NO hook clears residual `.ck-workflow-state.json`; `session-init` clears it only on explicit `/clear`.
