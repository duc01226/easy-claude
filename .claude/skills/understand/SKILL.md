...
**After a cutoff, compaction, or resume:** `TaskList` → read the ledger → **verify every `written` row against the filesystem** (the file exists AND carries its sections; an absent or truncated block resets to `pending`) → re-read the contracts → continue at the first unfinished group. NEVER restart a finished group, and never re-derive a written block from memory (`references/scale-protocol.md` §5).
...
- End there. **Do not** quiz, do **not** ask the developer to restate, do **not** call `AskUserQuestion`, do **not** wait for a reply to the §13 challenges, do **not** loop. **Never block the next workflow step.**
...
- **Wrap-up handoff:** `/watzup` invokes `/understand` after its session summary only for a large code change or when the user asks (the threshold lives in `/watzup`), so the developer gets the full report on the completed work without losing `/understand` as a standalone command.
