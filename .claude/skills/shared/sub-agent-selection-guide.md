...
`workflow-review-changes` is the canonical wave partition in this repo: it declares its waves as `parallelGroups` in `.claude/workflows.json:454-475`, and fixes the member → `subagent_type` mapping in that workflow's `preActions.injectContext` (`.claude/workflows.json:478`).
...
| Self-contained brief       | Each member gets: goal · exact scope + owned files · reference docs to read · a **unique exact artifact** path · return contract (summary + `Full report:` path, per `SYNC:subagent-return-contract`) · incremental persistence to `tmp/reports/` (per `SYNC:incremental-persistence`). |
| Specialist routing         | Every member routed via the decision table above — never `code-reviewer` as a catch-all.                                                                                                                                     |
| Design brief attached      | Any member that CREATES or RESHAPES a user-facing surface receives the **Design Plan** with its brief — colour (4–6 named hex) · type (families + roles + scale) · layout (concept + alignment) · principles — plus the design-system / SCSS / token doc paths it must adopt, and the reviewed `Generic test:` line. **A sub-agent inherits knowledge ONLY from its own agent definition and its brief**, so a UI member briefed without the plan supplies its own defaults and returns a templated result the parent then has to re-do. Where no design plan exists yet, say so and name who decides — never leave the axis silently to the leaf. |
...
| Max      | 2 rounds                                    | Then escalate to user via `AskUserQuestion`           | —                      |
