...
| Accessibility/responsive/visual (when relevant) | Applicable states, viewports/devices, design authority, the resolved `uiStateCapture.mode`, and (unless that mode is `off`) the UI-state-changing action inventory are declared | Each relevant state and viewport is observed; state-changing transitions follow the resolved `uiStateCapture.mode` — `every-action` observes each one, `declared-only` lists each as a recorded blind spot, and `off` records transition coverage once as `N/A — uiStateCapture off: {reason}` | Controls remain usable/readable/responsive; the state × viewport matrix is captured and indexed in a manifest in every mode, and transition captures are emitted from the shared action layer only while the mode captures them; every artifact is opened/read and recorded case by case before synthesis; a missing matrix capture keeps the row `ENVIRONMENT-BLOCKED`/`UNVERIFIED`, never `PASS`; static findings route to UI review and runtime findings to experience review. Detail: `.claude/skills/shared/ui-state-capture-protocol.md` |
