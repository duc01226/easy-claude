...
> **[IMPORTANT] Database Performance Protocol (MANDATORY, MUST ATTENTION):**
>
...
**Findings-Validation Gate (MANDATORY before any fix):**

Before fixing anything, route the Pass 1 + Pass 2 findings through `/why-review --validate-findings <report-path>` to confirm each is real — eliminating false positives that would otherwise drive needless or wrong changes. Fix ONLY findings the gate validates; discard or downgrade the rest with a recorded reason. (Same false-positive control as `/security-review` and `/plan-review`.)

Then fix the validated actionable issues.
...
- Auto-triggered by workflow orchestration after `/feature-implement`, `/fix`, `/plan-execute`
...
> When the review finds 10+ changed files, apply the **Systematic Review Batching** protocol (map-reduce: size-capped batches + hierarchical synthesis) defined below.
