...
For user-facing UI creation/reshaping apply, in order, journey-first `UX-1`–`UX-11`, usability/accessibility `UI-1.1`–`UI-9.4` and identity `DD-1`–`DD-8`; read `.claude/docs/ux-journey-process.md`, `.claude/docs/design-knowledge.md` and `.claude/docs/design-review-checklist.md` first. Before any design output, report the main user journeys (actors, jobs, ranked journeys with step tables, derived requirements), then read project design principles/design system/existing UI; only then name subject/audience/job; write a reasoned colour/type/layout/principles Design Plan, run the blocking similar-prompt generic test and revise defaults before building. Critique the built page, remove one accessory and walk every main journey on it (`UX-8`), measure interaction cost and wayfinding (`UX-9`, `UX-10`), and close with a UI/UX Gate Report covering UX/UI/DD/CL/copy (`UX-11`). Explicit brief wins, then established project design system/ADRs; surface genuine conflicts. Carry the journey report, plan and design-system paths into UI agent briefs. State N/A only when no visual surface changes.
