...
| `golden-rules`        | Golden Rules         | `contextGroups[].rules` (all groups merged); `portability.inlinePathRules: false` renders group names + conventions-hook and `--lookup` pointer instead of rule text only when `conventionInjection.enabled: true` AND the conventions lib is available (every rule-bearing group named, unique and ranked within `maxClassesPerEdit`; worst-case digest within `maxChars`); otherwise inline + generator `[WARN] INLINE_PATH_RULES` | `buildGoldenRules`        | Yes — skip if no contextGroups or all rules[] empty |
