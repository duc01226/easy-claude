...
description: '[Documentation] Use when you need to create or update business feature documentation. Renamed from the former /feature-docs skill — that name no longer resolves as a slash command.'
---
...
**Goal:** Generate tech-free 8-section business Feature Specs; code evidence is carried only in the Section 8 test-case anchors, never in prose.

> **Renamed:** formerly `/feature-docs` — that name no longer resolves as a slash command; use `/feature-spec`.

**Modes:**

| Mode     | Trigger                                      | Input                      | Output                               |
| -------- | -------------------------------------------- | -------------------------- | ------------------------------------ |
| `init`   | No `docs/specs/{Bucket}/` exists | Full source code           | Complete 8-section tech-free Feature Spec from scratch |
...
3. **Write Documentation** — Follow the 8-section tech-free structure, place in `docs/specs/{Bucket}/`
...
> **Scale gate for INIT mode (workflow context only):** If `workflow-spec-driven-dev` is the caller AND module_count ≥ 4 → MUST spawn sub-agents (one per module) in ONE message. When feature-spec is invoked STANDALONE for a single module, no scale gate applies — single-module init is always single-session.

> **Mode: AMEND (bugfix scope — NOT a re-author).** Triggered by `feature-spec [mode=amend]` from the bugfix workflow (inserted after `debug-investigate`, before `plan`). The Feature Spec already exists; a bug fix changes a narrow slice of behavior. Do the MINIMUM — touch only the sections the bug touches:
>
...
After extraction is complete, fold the extracted content into the 8 business sections. Technical artifacts (commands/handlers, API routes, message/event contracts, UI component names) are the code's responsibility and are NOT written into the doc — they only *inform* the business sections and test cases below:
...
> **[BLOCKING] TC Format:** Use canonical format in `.claude/skills/shared/tc-format.md`. NEVER use abbreviated flat GIVEN/WHEN/THEN — use full template with all required fields (Objective, Preconditions, GWT steps, Acceptance Criteria, Test Data, Edge Cases, Evidence, IntegrationTest, Status). Section 8 owned exclusively by `/spec-tests` — feature-spec populates only during INIT. Existing TCs MUST NOT be overwritten during UPDATE mode.
...
- **Size caps** — body (sections 1-7) ≤1200 lines, whole file ≤1800 (hard). **Split** the capability when body>1200 OR TCs>40 (or when two distinct module-level capabilities emerge):
    ...
    5. Update `INDEX.md` to link both parts
...
## Phase 3: Bucket Index Update

After creating/updating Feature Specs, update `docs/specs/{Bucket}/INDEX.md`:

1. Read current content
2. Verify each Feature Spec in the bucket is listed
3. Add or refresh rows using flat Feature Spec links:
    ```markdown
    | [{FeatureName}](README.{FeatureName}.md) | {summary} | {feature_code} | {TC count} | {status} |
    ```
...
>
> 1. **Activate `feature` workflow** (Recommended) — spec-driven with tests by default: scout → investigate → domain-analysis → why-review → feature-spec → plan → plan-review → plan-validate → why-review → spec-tests → why-review → review-artifact --type=spec-tests → plan → plan-review → cook → review-domain-entities → spec-tests → why-review → review-artifact --type=spec-tests → spec-tests [direction=sync] → integration-test → integration-test-review → integration-test-verify → workflow-review-changes → sre-review → security-review → changelog → test → docs-update → workflow-end → watzup
> 2. **Execute `/feature-spec` directly** — run this skill standalone
...
- **"/spec-tests (Recommended)"** — Generate/update test specs for documented features
- **"/spec-tests [direction=sync]"** — Sync test specs to dashboard
...
| ---------------------------- | ----------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `/spec-index`                | **Derived consumer** — assembles a regenerable navigation index/ERD FROM these Feature Specs (never a source of truth) | AFTER specs exist — run to (re)generate the bucket `INDEX.md` / cross-capability ERD over the canonical specs |
| `/spec-tests`                  | **Consumer** — reads Section 8 to generate/validate TCs                                         | After creating/updating feature docs, call spec-tests to populate or review Section 8                 |
| `/review-artifact --type=spec-tests`           | **Reviewer** — audits TC coverage in Section 8                                                  | After spec-tests, to validate TC completeness and GIVEN/WHEN/THEN quality                             |
| `/spec-tests [direction=sync]` | **Sync** — forward-syncs Section 8 TCs ↔ integration test code (§8 canonical; test code implements it) | After spec-tests update, to reconcile §8 TCs with the integration test suite                          |
| `/integration-test`          | **End consumer** — generates test code from TCs in Section 8                                    | After spec-tests, to produce actual integration test files                                            |
| `/docs-update`               | **Orchestrator** — calls this skill as Phase 2                                                  | Run /docs-update for full chain sync; it calls /feature-spec internally                             |
...
```
feature-spec (you are here)
  │
  ├─ AFTER this skill:
  │
  ├─ [REQUIRED] → /spec-tests [CREATE or UPDATE mode]
  │     Section 8 is incomplete without explicit spec-tests run.
  ...
  └─ [RECOMMENDED] → /docs-update
        ...
        spec-index (Phase 2.5) and spec-tests (Phase 3).
```
...
- **IMPORTANT MUST ATTENTION [BLOCKING]** EVERY test case MUST carry verifiable code evidence as a `[Source: namespace/service/id]` abstract anchor in its Section 8 hidden carrier — no exceptions (physical `file:line` → provenance sidecar only)
...
- **IMPORTANT MUST ATTENTION [BLOCKING]** Existing TCs MUST NOT be overwritten during UPDATE mode — Section 8 owned by `/spec-tests` after INIT
- **IMPORTANT MUST ATTENTION [REQUIRED]** 8 tech-free sections MUST match master template in exact order; prose contains zero technical terms (code is the technical source of truth)
