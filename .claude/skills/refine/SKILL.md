...
name: refine
version: 2.3.1
description: '[Project Management] Use when a workflow step or the user asks for idea refinement. Converts ideas to PBIs, validates problem hypotheses or adds acceptance criteria.'
---
...
1. Glob `*/plan.md` under the plans root (default `plans/`; a `docsRoots.plans.path` entry in `docs/project-config.json` overrides the path) sorted by modification time, or check `TaskList` for plan context
