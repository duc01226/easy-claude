...
- Hook inventory reduced to 52 top-level hook files + 33 lib modules — the dispatcher consolidation above, plus removal of two unregistered scratch hooks (`_fix2.cjs`, `_fix_suggester.cjs`) that had zero inbound references and were absent from `settings.json`. Counts reconciled across `README.md`, `CLAUDE.md`, `AGENTS.md`, `docs/project-reference/project-structure-reference.md`, `.claude/docs/README.md`, `.claude/docs/hooks/README.md` (incl. PreToolUse `30→24`, SubagentStart `8→3`), `.claude/docs/team-collaboration-guide.md`, and `.claude/docs/claude-ai-agent-framework-guide.md`. A broader pre-existing doc-count drift (subagent-init `8`/`13`, top-level `64`/`66` in `quick-start.md`, `troubleshooting.md`, `domain-entities-reference.md`, `learn/SKILL.md`, the explainer scenes, and the Codex/Copilot mirrors — plus skill/lib counts) is tracked for a separate reconciliation pass.
